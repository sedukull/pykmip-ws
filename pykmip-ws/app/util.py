...
def check_auth(user_name, password):
    ...
    db_obj = KisDb()
    return db_obj.verify_app_cred(generate_hashed_str(user_name), generate_hashed_str(password))
...
def generate_hashed_str(inp_str):
    return base64.b64encode(hashlib.sha512(str(inp_str)+str(random.getrandbits(512))).digest(),base64.b64encode(hashlib.sha512(str(random.getrandbits(512))).digest(),random.choice(['rA', 'aZ', 'gQ', 'hH', 'hG', 'aR', 'DD'])).rstrip('=='))
